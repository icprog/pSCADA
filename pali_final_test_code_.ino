...
void loop()
{
  ...
  outputValue = map(sensorValue,220 , 1023, 0, 54); // 21
  ...
  Serial.print("GSM Count ");
  Serial.println(gsmCount);
if(gsmCount==1)
             {
  ...
  float testValue = outputValue * 100; // 2100
  float ltSec = testValue / 360; // 5.833 Curretn Flow
  ...
  //*********************Counter to send value in EEPROM************************************

  Serial.print("Tsec Value = ");
  Serial.println(cumm12);
